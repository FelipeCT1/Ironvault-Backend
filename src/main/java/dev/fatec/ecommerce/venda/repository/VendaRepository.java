...
public interface VendaRepository extends JpaRepository<Venda, Long> {
    ...
    @Query("SELECT v FROM Venda v LEFT JOIN FETCH v.itens WHERE v.codigoPedido = :codigo")
    Optional<Venda> findByCodigoPedidoFetched(@Param("codigo") String codigoPedido);

    @Query("SELECT v FROM Venda v LEFT JOIN FETCH v.itens WHERE v.clienteId = :clienteId ORDER BY v.dataCriacao DESC")
    List<Venda> findByClienteIdFetched(@Param("clienteId") Long clienteId);

    @Query("SELECT v FROM Venda v LEFT JOIN FETCH v.itens ORDER BY v.dataCriacao DESC")
